...
export const description = 'List accounts a user is following';
...
export const options = zod.object({
	...
	limit: zod
		...
		.describe(
			option({
				alias: 'l',
				description: 'Maximum number of following to return',
			}),
		),
	all: zod
		.boolean()
		...
		.describe(
			option({
				description: 'Fetch all following (paginate automatically)',
			}),
		),
	sort: zod
		.string()
...
export default function Following({args: [user], options}: Properties) {
	const run = useCallback(
		async (client: InstagramClient) => {
			...
			let following;
			try {
				following = await paginate(fetchPage, {
					all: options.all,
					limit: options.limit,
			...
			const filtered = applyFilters(
				following as unknown as Record<string, unknown>[],
				{
					limit: options.limit,
